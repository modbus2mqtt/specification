...
export class NumberConverter extends Converter {
    ...
    modbus2mqtt(spec: Ispecification, entityid: number, value: ReadRegisterResult): number | string {
        ...
        if (entity) {
            ...
            let numberFormat = (entity.converterParameters != undefined && (entity.converterParameters as Inumber).numberFormat != undefined) ? (entity.converterParameters as Inumber).numberFormat : EnumNumberFormat.default
            let v = (numberFormat == EnumNumberFormat.float32) ? value.buffer.readFloatBE() : value.data[0]
    ...
    override getModbusLength(entity: Ientity): number {
        if (entity.converterParameters == undefined || (entity.converterParameters as Inumber).numberFormat == undefined)
            return 1;
